...
; 10-APR-2000 The documentation is not correct.  REM_DUP uses the IDL sort
;routine to first sort the data.  The IDL sort routine does not maintain the
;original order of the subscripts when more than one item has the same value. If
;it is important to find the subscript of the first occurence, change line 62:
;
;	sub = sort(a) to sub = bsort(a) Don
;-
;-------------------------------------------------------------------------------
;
 On_error,2
 ...
 sub = bsort(a)			;sorted subscripts 00apr10 - rcb/djl
 aa = a(sub)			;sorted a
