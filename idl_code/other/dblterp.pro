pro dblterp, Xtab, Ytab, Xint, Yint, MISSING = missing, NoInterp = NoInterp
;+
; NAME:   
;       dblTERP  
; PURPOSE: 
...
; EXPLANATION:
;       The results of dblTERP are numerically equivalent to the RSI
;       INTERPOL() function, but note the following:
;         (1) dblTERP is a procedure rather than a function
;         (2) INTERPOL() extrapolates beyond the end points whereas dblTERP
;             truncates to the endpoints (or uses the MISSING keyword)
;         (3) dblTERP (unlike INTERPOL) uses the intrinsic INTERPOLATE function
;                 and thus may have a speed advantage
;         (4) dblTERP always converts the new grid vector to floating point 
...
;
; CALLING SEQUENCE:
;       dblTERP, Xtab, Ytab, Xint, Yint, [MISSING =, /NoInterp ]   
;
; INPUT PARAMETERS: 
...
;       /NoINTERP - If supplied then dblTERP returns the YTAB value(s) 
...
;   
;       IDL>  dblTERP, WAVE, FLUX, WGRID, FGRID  
;
...
;	Improve accuracy of linterp - RCB Dec 2012
;-
 On_error,2

 if N_params() LT 4 then begin
   print,'Syntax - dblTERP, Xtab, Ytab, Xint, Yint, [ MISSING = ]' 
 ...
 zparcheck, 'dblTERP', Xtab, 1, numeric, 1, 'Current X Vector' 
 zparcheck, 'dblTERP', Ytab, 2, numeric, 1, 'Current Y Vector' 
 zparcheck, 'dblTERP', Xint, 3, numeric, [0,1], 'New X Vector or Scalar'
