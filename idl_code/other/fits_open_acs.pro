...
; ********************RCB 03apr30****************
;this version is required to read the bad ACS files.
;	i should go thru the ACS data w/ the above no_abort option &
;	re-extract these bad ones from the archive so the new fits_open
;	in stiscal will work. Also & otherwise, i need to use this w/ the
;	old ACS Fits_read!
;**************************************************************************

; 03JAN23 - this version copied from /data/rcbsun1/astrolib/pro
; which has the fix for bad SBC files. Placed here to avoid getting the 
; STIS pro version, which fails for ACS/SBC.
; another solution would be:
;Tue, 21 Jan 2003 16:22:07 -0500 (EST)
;        Don Lindler <lindler@rockit.gsfc.nasa.gov>
;The cleanest way to fix your problem is to get rid of the bad FITS
;files.  Otherwise I would create a file acs_idl with
;       setenv IDL_PATH ...
;        idl
        ;and in my .cshrc file I would add the line:
;alias acsidl "source /home/bohlin/acs_idl"
; 03may15 - problem fixed by re-extracting only 2 internal flats from archive!!
;
;
;+
; NAME:
...
;             files 
;	Lindler, Apr, 2002, Modified to read invalid files where
;		keyword XTENSION is not the first keyword in the
;		extension headers.
;-
;--------------------------------------------------------------------
;
