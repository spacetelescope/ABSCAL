PRO HRSobs,FILE,obs,det,grat,aper,star,seldet,selgrt,selapr,selstr
;+
;
; Routine to read output of HRSDIR.PRO ASCII FILES
; 96APR1-rcb
;
; Input: file - name of file output by HRSdir w/ list of obs to process
; optional input:	
...
; optional output:
;	det, grat, aper, and STAR name- 1D arrays of corresponding HRS config
;
; EXAMPLE: HRSobs,FILE,obs,det,grat,aper,star,seldet,selgrt,selapr,selstr
; HISTORY
;	96APR1-RCB ADAPTED FROM FOSOBS
;-------------------------------------------------------
;
; open file
;
...
;
; read until hitting Z as first character
;
	obs=strarr(1000)
	DET=OBS
	GRAT=OBS
	...
	star=obs
	st=''
	while strmid(st,0,1) ne 'Z' do readf,1,st
;
; loop on records
;
	n=-1
...
IF N EQ -1 THEN BEGIN
	PRINT,'*****WARNING***** NO MATCHES FOR REQUEST IN HRSOBS for ',       $
