...
st=''
!y.style=1
fils=findfile('spec/spec_i*pn.fits')
	filspn=fils					; save for Z-orders
...
readcol,'pnref/IC5117_rudy.txt',wrudinput,frud	; IC5117-Rudy vac, no Vr corr
; NO airtovac wrud already vac---

close,6  &  openw,6,'wlmeas.temp'
...
; 256 is sat., 512 FF glitches, 32-CTE tail Table 2.5
	...
	fdecomp,fils(ifil),dsk,dir,root
...
; analyze lines:
		...
		xgood=where(wl/iord gt wlref(ilin)-dw and 		$
						wl/iord lt wlref(ilin)+dw)
	    	!mtitle=grat+' '+name+' Order,line='+			$
					string([iord,wlref(ilin)],'(i2,f8.1)')
		mx=max(net(xgood))
		maxpos=where(mx eq net)
		null_plot,xpx,pltnet,0,psym=-8,xr=[xgood(0),max(xgood)],$
						yr=[0,mx],th=2
...
; bad lines w/ eyeball centers 256 is sat. 32-CTE tail Table 2.5
   ...
   if name eq 'ic6907ciq' and iord eq -1 and ilin eq 5 then xline(ilin)= 69.6
		...
		if ilin eq 2 and iord  eq 1 then begin		; brite 10830A
		    wlimaz,name,z.y,z.wave,xcen,ycen
		    xline(2)=xcen
...
; ###change
				if ilin eq 4 then read,st ; extra SED reminder
				endif
; ###change 2013may27-retool -- start over w/ better centroiding:
		    badflag=0
		    if xline(ilin) eq 0 then begin	; comment to ignore bads
			del=max(xgood)-xgood(0)
			cont=(avg(net(where(xpx ge xgood(0) and 	$
			    xpx le xgood(0)+del/5)))+  			$
    			    avg(net(where(xpx ge max(xgood)-del/5 and 	$
			    xpx le max(xgood)))))/2
		...
		if badflag eq 1 then read,st
; ###change
;		if strpos(fils(ifil),'_ic6906c0q') ge 0 then read,st	; test
		if ilin eq 4 then read,st
		if st ne '' then begin
redo:		   	print,'Enter Line Center Value'
			...
			read,xcentr
		   	xline(ilin)=xcentr
			oplot,[xline(ilin),xline(ilin)],[0,99999],lin=1
		   	print,"   if name eq '"+name+"' and iord eq "	$
			  +string(iord,'(i2)')+' and ilin eq '+		$
			  string(ilin,'(i1)')+				$
			   ' then xline(ilin)='+string(xcentr,'(f5.1)')
			read,st
			if st ne '' then goto,redo
			endif
skiplin:
			endfor					; end line loop
	    printf,6,name,star,grat,zxpos,zypos,iord,xline,		$
	    			form='(a9,1x,a7,a5,2f8.2,i3,1x,9f8.2,1x)'
